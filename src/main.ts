...
export function rotatePointRad(p: Vector, angleRad: number, origin: Vector = { x: 0, y: 0 }): Vector {
  ...
  return { x: xr + origin.x, y: yr + origin.y };
}
...
function driveTankRobot(dt: number) {
    ...
    v = rotatePointRad(v, to_rad(90));
    // v = rotatePointRad(v, to_rad(-90)); // undo operation

    console.log(
        "x: " + v.x.toFixed(0) + "\n", 
        "y: " + v.y.toFixed(0) + "\n", 
        "A: " + tankRobot.get_angle().toFixed(0)) + "\n";
}
